...
def test_c2e_solve(request):
    ...
    cfg = fmm.make_config(K_name, params, 1.0, 1.0, 1, float_type, alpha = 1e-10)
    op = c2e_solve(cfg.gpu_module, s, b, 3.0, 1.1, cfg.K, params, cfg.alpha, float_type)
    return op
...
def test_scaling_relations():
    ...
    K_name = 'laplaceD2'
    cfg = fmm.make_config(K_name, [], 1.0, 1.0, 1, float_type, alpha = 1e-8)
    op = np.array(c2e_solve(cfg.gpu_module, s, b, 3.0, 1.1, cfg.K, cfg.params, cfg.alpha, float_type))

    for i in range(5):
        b2 = Ball(np.random.rand(2).tolist(), 1.0)
        op2 = np.array(c2e_solve(
            cfg.gpu_module, s, b2, 3.0, 1.1, cfg.K, cfg.params, cfg.alpha, float_type
        ))
