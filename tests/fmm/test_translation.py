...
def test_scaling_relations():
    ...
    op = np.array(c2e_solve(gpu_module, s, b, 3.0, 1.1, K, params, float_type))

    for i in range(5):
        b2 = fmm.two.Ball(np.random.rand(2).tolist(), 1.0)
        op2 = np.array(c2e_solve(gpu_module, s, b2, 3.0, 1.1, K, params, float_type))
        np.testing.assert_almost_equal(op2, op)

    scale = 2.0
    b2 = fmm.two.Ball([0,0], scale)
    op2 = np.array(c2e_solve(gpu_module, s, b2, 3.0, 1.1, K, params, float_type)) / scale
    np.testing.assert_almost_equal(op2, op)
