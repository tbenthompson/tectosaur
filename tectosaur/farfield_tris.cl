    ...
    if (obs_tri_idx < n_obs) {
        ...
        for (int j = 0; j < n_src; j++) {
            ...
            for (int iq = 0; iq < n_quad_pts; iq++) {
                ...
                const double CsRT0 = -1.0 / (8.0 * M_PI * (1.0 - nu));
                ...
                for (int b_src = 0; b_src < 3; b_src++) {
                    ...
                    % for Ip in range(3):
                        Kval += CsRT0 * invr * (
                            (1 - 2 * nu) * ${e[Ij][d_obs][d_src]}
                            - ${e[Ij][Ip][d_src]} * invr * invr * 
                                D${dn(Ip)} * D${dn(d_obs)}
                ...
                for (int b_src = 0; b_src < 3; b_src++) {
                    ...
                    % for Ip in range(3):
                    Kval += CsRT1 * (
                        D${dn(Ip)} * invr * invr * invr * nsrc${dn(Ip)}
                    );
                    % endfor
                    sum[b_obs * 3 + ${d}] += Kval *
                        obs_jacobian * src_jacobian * quadw
                        * obsb[b_obs] * srcb[b_src] * in[b_src * 3 + ${d}];
                }
                }
                %endfor
            }
