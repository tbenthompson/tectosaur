...
{
    auto obs_split_pt = get_split_pt(obs_tri, min_angle_isoceles_height);
    auto obs_split_pt_xyhat = xyhat_from_pt(obs_split_pt, obs_tri);
    assert(check_xyhat(obs_split_pt_xyhat));

    auto src_split_pt = get_split_pt(src_tri, min_angle_isoceles_height);
...
py::tuple separate_tris_pyshim(const Tensor3& obs_tri, const Tensor3& src_tri,
        double min_angle_isoceles_height) 
{
    auto out = separate_tris(obs_tri, src_tri, min_angle_isoceles_height);
