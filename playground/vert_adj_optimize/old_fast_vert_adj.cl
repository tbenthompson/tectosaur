...
<%def name="calc_quad_pt_contrib(k_name)">
    ...
    % for d_obs in range(3):
        % for d_src in range(3):
            kernel_val = quadw * K${d_obs}${d_src};
            % for b_obs in range(3):
                % for b_src in range(3):
                    idx = ${prim.temp_result_idx(d_obs, d_src, b_obs, b_src)};
                    val = obsb${b_obs} * srcb${b_src} * kernel_val;
                    y = val - kahanC[idx];
                    t = result_temp[idx] + y;
...
for (int i_b = 0; i_b < ${nq}; i_b++) {
    ...
    % if lim_idx == 0:
    Real alpha_max = 0.5 * rho_max(tp) / cosb;
    % else:
    Real alpha_max = 0.5 * rho_max(tq) / sinb;
    % endif
